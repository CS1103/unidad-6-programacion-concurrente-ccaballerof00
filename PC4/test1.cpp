...
SCENARIO("Cálculo de la diagonal de la matriz que resulta de la multiplicación no concurrente de 2 matrices")
{
    ...
    )
    {
        ...
        {
            Matriz<int> m1=Matriz<int>(3,4);
            for(int i=0;i<3;i++){
                for(int j=0;j<4;j++){
                    m1.AnadirElemento(2,i,j);
                    }
                }
            Matriz<int> m2=Matriz<int>(4,5);
            for(int i=0;i<4;i++){
                for(int j=0;j<5;j++){
